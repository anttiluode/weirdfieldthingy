...
Complete Full Reset and Continuous Simulation

This application creates a dynamic 2D field that continuously evolves using a
simple convolution–decay–noise update rule. At each update step, an external image
(from a webcam) is fed in as a bias to the field update. The GUI (built with Tkinter)
...
There is now no start/stop simulation or “reset field” button.
...
class DynamicField(nn.Module):
    def __init__(self, field_size=64, coupling=0.1, decay=0.995, noise_scale=0.02,
                 input_scale=0.2, init_mean=0.0, init_std=0.1, device='cpu'):
        """
        Initialize a dynamic field of size (field_size x field_size) on the given device.
        The field is updated via a simple convolution, decay, and noise injection.
        An external input (e.g., from a webcam) is added (scaled by input_scale) as bias.
        
        The field is initialized using a random normal distribution with mean (init_mean)
        and standard deviation (init_std).
        """
        super().__init__()
        ...
        # Default parameters for resetting settings
        self.default_field_size = field_size
        self.default_coupling = coupling
        self.default_decay = decay
        self.default_noise_scale = noise_scale
        self.default_input = input_scale
        self.default_init_mean = init_mean
        self.default_init_std = init_std
        ...
        # Simple 3x3 convolution for local coupling
        self.conv = nn.Conv2d(1, 1, kernel_size=3, padding=1, bias=False)
        nn.init.normal_(self.conv.weight, mean=0.0, std=0.1)
        self.conv.to(device)

    def evolve(self, image_input=None):
        """
        Update the field. If image_input is provided (a 2D numpy array in [0,1])
        then it is converted to a tensor and added (scaled by input_scale) as external bias.
        """
        conv_update = self.conv(self.field)
        if image_input is not None:
            input_tensor = torch.tensor(image_input, dtype=torch.float32, device=self.device)
            input_tensor = input_tensor.unsqueeze(0).unsqueeze(0)
            input_contrib = self.input_scale * input_tensor
        else:
            input_contrib = 0
        noise = torch.randn_like(self.field) * self.noise_scale
        new_field = torch.tanh(self.decay * self.field + self.coupling * conv_update + input_contrib + noise)
    ...
    def reset_field(self):
        ...
        """
        self.field.data = torch.randn(1, 1, self.field_size, self.field_size, device=self.device) \
                           * self.init_std + self.init_mean

    def set_fidelity(self, new_size):
        """Adjust the resolution (fidelity) of the field via bilinear interpolation."""
        old_field = self.field  # shape: (1,1,H,W)
    ...
    def load_state(self, state):
        ...
        self.coupling = state.get('coupling', self.coupling)
...
class FieldWebcamGUI:
    def __init__(self, root, simulator: DynamicField):
        ...
        self.root.title("Dynamic Field Webcam App")
        self.status_var = tk.StringVar(value="Simulation Running")
        self.webcam_index_var = tk.StringVar(value="0")  # Default webcam index as string
        self.cap = None
        self.running = True  # Simulation always runs
        self.setup_gui()
    ...
    def setup_gui(self):
        ...
        # New Reset button: this completely restarts the program (including Tkinter)
        ...
        ttk.Label(control_frame, text="Coupling:").pack(side=tk.LEFT, padx=5)
        self.coupling_slider = ttk.Scale(control_frame, from_=0.01, to=0.5, orient=tk.HORIZONTAL, command=self.change_coupling)
        self.coupling_slider.set(self.simulator.coupling)
    ...
    def initialize_camera(self, index):
        if self.cap is not None:
            self.cap.release()
        # Use DirectShow backend (cv2.CAP_DSHOW) to reduce warnings on Windows.
    ...
    def change_coupling(self, value):
        self.simulator.coupling = float(value)
        self.status_var.set(f"Coupling set to {float(value):.3f}")
    ...
    def simulation_loop(self):
        while self.running:
            ...
            if ret:
                ...
                normalized = None
            self.simulator.evolve(image_input=normalized)
...
def main():
    ...
    simulator = DynamicField(field_size=64, coupling=0.1, decay=0.995, noise_scale=0.02,
